...
- (void)loadAllgoodsDataArray{
    ...
    [MCNetTool postWithCacheUrl:HttpShopClass params:params success:^(NSDictionary *requestDic, NSString *msg) {
        ...
        self.bottomViewH.constant = 35 * self.class1_listArray.count + 200;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.categoryTableView){
        ...
        cell.tableview_textLabel.backgroundColor = [UIColor whiteColor];
        if (indexPath.row  > 3){
            [self.scrollview setContentOffset:CGPointMake(0, indexPath.row * 35 - 70) animated:YES];
        }
        [UIView animateWithDuration:0.25 animations:^{
            self.suspendTableTopDis.constant = indexPath.row * 35;
        }];
