...
@implementation EngineerHeaderReusableView
...
- (CLHomeHeader *)cLHomeHeader{
     if (!_cLHomeHeader) {
       _cLHomeHeader = [[CLHomeHeader alloc] initWithFrame:CGRectMake(0, 0, kScreenHeight, 60)];
         _cLHomeHeader.backgroundColor = [UIColor whiteColor];
//        _cLHomeHeader.newses = @[@[@"第0组第一行",@"第0组第二行"],@[@"第一组第一行",@"第一组第二行"],@[@"第二组第一行",@"第二组第一行"],@[@"第3组第一行",@"第3组第二行"]];
        _cLHomeHeader.frame= CGRectMake(0,0, kScreenWidth, _adView.height);
         ...
         _cLHomeHeader.clickCellBlock = ^(NSString * t_id ){
              if (weakSelf.clickEngineerHeaderReusableViewBlock) {
                 weakSelf.clickEngineerHeaderReusableViewBlock(t_id);
             }
          };
