...
#define maxScale 2.0
#define minScale 1.0
#define kAScreenWidth ([UIScreen mainScreen].bounds.size.width)
#define kAScreenHeight ([UIScreen mainScreen].bounds.size.height)

@interface AvatarBrowser ()<UIScrollViewDelegate,UIActionSheetDelegate>
...
- (instancetype)initWithImage:(UIImage *)orignImage view:(UIView *)orignView;{
    ...
    return [self initWithFrame:frame image:orignImage view:orignView];;
}
...
- (instancetype)initWithFrame:(CGRect)frame image:(UIImage *)orignImage view:(UIView *)orignView;
{
    ...
    if (self) {
        ...
        self.alpha = 0;
        UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, kAScreenWidth, kAScreenHeight)];
...
- (void)show
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.imageView.frame = CGRectMake(0 , (kAScreenHeight - imageH * kAScreenWidth / imageW) /2, kAScreenWidth, imageH * kAScreenWidth/imageW);
        self.alpha=1;
