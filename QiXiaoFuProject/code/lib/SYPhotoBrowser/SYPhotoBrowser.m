...
- (void)addTools
{
    if (_titles) {
        ...
        _pageLabel.text = [NSString stringWithFormat:@"%ld/%ld", _currentIndex+1, (unsigned long)_originalImages.count];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    _pageLabel.text = [NSString stringWithFormat:@"%.0f/%ld", pageNum+1, (unsigned long)_originalImages.count];
    if (!_titles) {
        [_pageLabel sizeToFit];
        _pageLabel.frame = CGRectMake((SYScreenWidth-_pageLabel.frame.size.width-10)/2, SYScreenHeight-40, _pageLabel.frame.size.width+10, 30);
        _pageLabel.layer.cornerRadius = 6;
        ...
        for (UIView *view in views) {
            if ([view isMemberOfClass:[SYPhotoView class]]) {
                ...
                if (photoView.tag != index+1000) {
                    ...
                    LxDBAnyVar(photoView.imageView.frame);
